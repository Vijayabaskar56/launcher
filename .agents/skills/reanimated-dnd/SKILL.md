...
function SortableListExample() {
  ...
  return (
    <Sortable data={items} renderItem={renderItem} itemHeight={ITEM_HEIGHT} />
  );
}
```
...
<DropProvider ref={providerRef}>{/* ... */}</DropProvider>;
...
```tsx
<SortableGridItem id={item.id} data={item} activationDelay={250} {...props}>
